...
RUN conda install --yes -c anaconda jupyter pandas numpy scikit-learn statsmodels seaborn gensim
RUN conda install --yes -c conda-forge matplotlib holoviews imbalanced-learn tensorflow keras pyspark wordcloud mlxtend rise
RUN conda install --yes -c bokeh bokeh
RUN conda install --yes -c spacy spacy
