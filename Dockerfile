...
RUN pip install pycep-correios

RUN conda update -n base -c defaults conda
RUN conda create -n env python=3.6
RUN conda install --yes -c anaconda jupyter pandas numpy scikit-learn statsmodels seaborn gensim nltk joblib
RUN conda install --yes -c conda-forge matplotlib holoviews imbalanced-learn tensorflow keras pyspark wordcloud mlxtend folium pyldavis rise
